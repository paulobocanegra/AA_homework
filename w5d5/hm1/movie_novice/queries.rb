...
def find_angelina
  ...
  # Actor.where('name = ?', 'Angelina Jolie')[0].map { |k, v| [k, v] }
  angie = Actor.find_by(name: 'Angelina Jolie')
  puts angie
  angie
end
...
def below_average_years
  ...
  #with the count of movies scoring under 5 aliased as bad_movies, .group('yr')
  #in descending order
...
# Failure/Error: subject { top_titles.as_json }
     
#      ActiveRecord::StatementInvalid:
#        PG::GroupingError: ERROR:  column "movies.title" must appear in the GROUP BY clause or be used in an aggregate function
#        LINE 1: SELECT movies.title FROM "movies" HAVING (score > 9)
#                       ^
#        : SELECT movies.title FROM "movies" HAVING (score > 9)